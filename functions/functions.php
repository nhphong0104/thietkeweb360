...
function createLink($type="detail",$url=array(),$path="",$con_extenstion='html',$rewrite = 1){
	...
	switch($type){
      case "cat":
			...
			$menuReturn = '/' . $dat_rewrite . '/';
			break;
		case "cat_coban":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['new_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['new_title_rewrite'];
			$menuReturn = '' . $dat_rewrite . '-cb' . $dat_id . '/';
			break;
		case "cat_hotro":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['new_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['new_title_rewrite'];
			$menuReturn = '' . $dat_rewrite . '-ht' . $dat_id . '/';
			break;

		case "detail_library":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['sta_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['sta_title_rewrite'];
			$menuReturn = '/thu-vien/' . $dat_rewrite . '-lb' . $dat_id . '.html';
			break;
      case "test_done":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['sta_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['sta_title_rewrite'];
			$menuReturn = '/' . $dat_rewrite . '-td' . $dat_id . '/';
			break;
      case "detail_test":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['sta_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['sta_title_rewrite'];
			$menuReturn = '/test/' . $dat_rewrite . '-ts' . $dat_id . '.html';
			break;
      case "recruit":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['sta_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['sta_title_rewrite'];
			$menuReturn = '/test/' . $dat_rewrite . '-rs' . $dat_id . '.html';
			break;
		case "survey":
			$dat_id = isset($url['dat_id']) ? $url['dat_id'] : $url['sta_id'];
			$dat_rewrite = isset($url['dat_rewrite']) ? $url['dat_rewrite'] : $url['sta_title_rewrite'];
			$menuReturn = '/danh-gia/' . $dat_rewrite . '-sv' . $dat_id . '.html';
			break;
	}
	return $menuReturn;
